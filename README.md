...
Каждая лабораторная - это `sprint`, этап разработки по `agile`, под каждую отдельная именованная по теме ветка в `git`. Всего 4 репозитория: под два бекенда, фронтенд и нативное приложение. Каждая работа демонстрируется и защищается отдельно. При защите необходимо продемонстрировать работу приложения по своей теме, UML диаграмму из `StarUML`, репозиторий github с кодом и ответить на вопросы. По первому модулю необходимо также сделать ТЗ, а по второму отчет по курсу - РПЗ.

* [Демо версия](https://azat-bilalov.github.io/book-of-memory-frontend/) интерфейса создателя заявки
* [Примеры](/tutorials/homework) UML и ER диаграмм
* [Примеры](/docs/) документации
...
Обязательно наличие 4 таблицы: 
* `услуг` (статус удален/действует, изображение)
* `заявок` (статус, дата создания datetime, дата формирования datetime, дата завершения datetime, `создатель` и `модератор`)
* `м-м заявки-услуги` (составной первичный ключ)
* `пользователей`
...
* Мастер-класс [gORM и шаблоны Golang](https://www.youtube.com/watch?v=Hh9Lf__Wohw)
...
3. **задачи** - 12 задач (8 лабораторных, ДЗ, документация, репозиторий Git, демо в Pages) с указанием технологий по каждой задаче
...
5. **Функциональные требования** - список окон и какие действия для каких групп пользователей доступны. Указать, какие методы бэкенда при этом вызываются. Всего 9 пунктов - меню и 8 страниц: 
...
- **Порядок показа**: выполнить авторизацию через `swagger` в режиме инкогнито, использовать содержимое `куки`/`localStorage`+`authorization` из браузера для заголовков остальных запросов через `swagger` и `insomnia`/`postman`. Выполнить GET списка заявок: 401/403 для гостя, для создателя только его. Выполнить PUT завершения заявки: для создателя 403 статус, для модератора успех и обновление полей. Выполнить GET списка заявок - для модератора все заявки. Показать содержимое `Redis`
...
* Мастер-класс [Авторизация на Django](https://www.youtube.com/watch?v=02YL8EcGONY)
* [Методические указания Golang JWT](/tutorials/lab5-go/README.md)
* Мастер-класс [Авторизация, Swagger и Redis на Golang](https://www.youtube.com/watch?v=F4Kll9WFliE&t=1546s)

#### Лабораторная 6

- **Цель работы**: Завершение интерфейса `пользователя` в `React`
- **Порядок показа**: показать авторизацию, добавление и формирование `заявки` в режиме разработчика. Пояснить в коде использование `redux` и `axios`. Показать авторизацию в браузере, использовать содержимое `localStorage`/`cookie` чтобы показать заявки пользователя в `insomnia`/`postman`.
...
Добавить страницы для регистрации и авторизации. Добавить страницу для просмотра списка `заявок` пользователя в виде таблицы. Добавить в меню пункты для новых страниц. Для обращений к методам веб-сервиса использовать `axios`. При выполнении запросов отображать на странице анимацию.
...
- **Порядок показа**: Кроме `Tauri` показывать на телефоне, отредактировать услуги в БД и продемонстрировать изменение в нативном приложении, показать поиск и переход в Подробнее
...
* Мастер-классы [Tauri на основе проекта React](https://www.youtube.com/watch?v=5EbntrQyaAo) и [Tauri приложения с нуля](https://www.youtube.com/watch?v=EVzoZr5JNwo&t=16s)
* [Методические указания React Native + Redux Toolkit](/tutorials/react-native/react_native.md)
* Мастер-класс [React Native](https://www.youtube.com/watch?v=XN87dx7HWZg)
