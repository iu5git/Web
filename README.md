...
Для карточек таблицы услуг добавить кнопку удаления услуги с помощью обращения к методам модели.
...
* [Методические указания](/tutorials/lab2-py/lab2_tutorial.md)
...
Добавить метод для подсчета суммы (количества) покупок/бронирований за определенную дату
...
* [Методические указания](/tutorials/lab3-py/lab3_tutorial.md)
* [Методические указания](/tutorials/lab3-go/golang/README.md)
...
Добавление Ajax-запросов, менеджера состояний (`redux` или через хуки `useReducer`, `useContext`). Подключение разработанного интерфейса фронтенда к веб-сервису из лабораторной №3. Ajax-запросы написать самостоятельно (через fetch например).
...
Для подключения к нужным сервисам/методам бэкенда использовать автоматическую генерацию через swagger. На бэкенде должна быть доступна админская панель.

Добавление фильтрации и поиска на странице услуг (сама фильтрация на стороне бекэнда):
...
Контрольные вопросы: куки, сессия, redis
