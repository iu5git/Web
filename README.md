...
* `модератор` (если выполняется это задание): список услуг таблицей, редактирование/создание услуги
...
- **Порядок показа**: вызвать через `insomnia` GET списка заявок, затем http-метод асинхронного сервиса (в `insomnia` и отдельно кнопкой в интерфейсе), показать что в GET списка заявок появился результат с задержкой, потом вызвать новый метод основного сервиса с ключом, чтобы изменить результат. Сделать скриншоты всех изменений в РПЗ по 2025 году, скриншоты `TestVKR` и объема файла РПЗ.
...
#### Добавление WebLLM/transformer.js в интерфейс гостя

- **Задание**: Добавление в приложение React одной из двух нейронных сетей на выбор, запускаемых в браузере, для обработки текста в разработанном приложении

1. Добавление ассистента с помощью `WebLLM` для `гостя` на строне браузера (Chrome, Яндекс, Arc) для консультаций пользователя по описанию услуг на странице `Списка услуг`
2. На странице описания одной `услуги` добавить внизу список похожих на нее `услуг` на основе близости эмбеддингов `описаний`, для вычисления эмбеддингов использовать `transformer.js`
...
- Лабораторная работа 2 на NestJS (TS, Nest, Handlebars, Sequelize, куки), каждая методичка по +4 баллов
...
5. Что такое Django? Его назначение и возможности.
6. Что такое шаблонизация Django? Приведите примеры.
